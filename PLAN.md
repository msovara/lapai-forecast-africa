...
**Status:** **Phase 0 closed (2026-06-29).** C4E `n320_gt6` teacher runs on Lengau V100; five Jan 2023 weekly inits forecast with `t2m/tp/u10/v10` on the Africa eval grid; baseline scorecard at `reports/PHASE0_BASELINE_SCORECARD.json` (GCS truth, scored locally). Closure pipeline: `configs/phase0_baseline.yaml`, `scripts/run_phase0_closure.py`, `pbs/phase0_baseline_lengau.pbs`, `reports/PHASE0_CLOSURE.md`. Offline Lengau workflow uses CDS IC cache + earthkit regrid cache (laptop populate → rsync). Scaffolding, inference scripts, gate, PBS, tests, and **green CI** are implemented. Mvua evaluation protocol wired — ERA5 truth, 2023–2025 test window, 6h+daily, **Africa default domain** (`configs/eval.yaml`).

**Track A A1 (2026-08-06):** Coarsening gate **PASSED 30/30** after full + extend fine-tune — see `reports/TRACKA_A1_GATE.json`, `reports/TEAM_NOTE_A1_PASSED.md`. Next: A2 pruning. GraphCast Africa remains available as a parallel teacher pathway (`teachers/graphcast/`, `reports/GRAPHCAST_FIRST.md`). This document remains the contract for Phases 0–3.
