...
|---|------|----------|--------|
| **1** | Finish / package production AF **t2m** eval (frozen v5) | Until campaign exits | **RUNNING** (Cassava GPU1 — do not kill / do not hog) |
| **2** | **Laptop benchmark** (size, CPU step time, teacher vs student) | 0.5 day | **PARTIAL** — Cassava CPU proxy done; real laptop optional |
| **3** | **Status matrix** (`MVULA_CODE4EARTH_STATUS_MATRIX.md`) | 0.5 day | **DONE** (this cycle) |
| **4** | Document **65→3 / no 10-day free-run** limitation | 0.25 day | **DONE** — link [`TRACKB_STATE_CLOSURE.md`](TRACKB_STATE_CLOSURE.md) |
| **5** | Freeze v5 + package README / repro notes | 1–2 days | **TO COMPLETE** |
| **6** | Honest “achieved / remains” for handover | with P3–P5 | **STARTED** in status matrix |
...
**Kill criteria (stretch):** if GPU time or calendar slips, stop at P1+P3+P4 documentation freeze; ship AF t2m + limitation docs + laptop size/CPU proxy.
...
- [ ] Production campaign JSON/MD/figures (`TRACKB_T2M_EXPANDED.*`) committed  
- [ ] Short interpretation paragraph for NMHS readers  

### B — Laptop / deployment demo

- [x] Disk size student vs K1 teacher  
- [x] CPU-only forward timing (proxy host)  
- [ ] Optional: same bench on a real consumer laptop  
