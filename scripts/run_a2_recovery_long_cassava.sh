#!/usr/bin/env bash
# A2 longer recovery FT (PLAN: 2–5 epochs @ 5e-5) on Cassava.
# Reuses existing soft-mask ckpt; trains ~3 epochs (~7500+ steps).
set -euo pipefail
...
test -f models/teacher_pruned_masked.ckpt
test -d data/processed/lapai/era5_n96_2020_2021.zarr

echo "[A2 long] recovery fine-tune max_epochs=3 max_steps=12000 lr=5e-5"
python -u - <<'PY'
...
anemoi["output_root"] = "models/trackA_prune_recover_long_runs"
...
PY
rc=$?
echo "[A2 long] train exit=${rc}"

latest=$(find models/trackA_prune_recover_long_runs/checkpoint -name 'inference-last.ckpt' 2>/dev/null | sort | tail -n 1 || true)
if [[ -n "${latest}" ]]; then
  ln -sfn "$(readlink -f "${latest}")" models/teacher_pruned.ckpt
  ls -lh models/teacher_pruned.ckpt
fi
echo "[A2 long] done $(date -Is) rc=${rc}"
exit "${rc}"
