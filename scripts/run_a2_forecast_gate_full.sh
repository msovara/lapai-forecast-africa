#!/usr/bin/env bash
# A2 FULL forecast (CDS IC) -> score (GCS) -> gate vs A1 coarsen scorecard.
# Isolated _full output paths; does not overwrite smoke A2 artifacts.
...
echo "[A2 fc full] ckpt=${CKPT_REAL}"
echo "[A2 fc full] dataset=data/processed/lapai/era5_n96_2020_2021.zarr"
echo "${CKPT_REAL}" | grep -q '31676a83-4919-4741-acaa-118fced72eab'
echo "${CKPT_REAL}" | grep -q 'trackA_prune_runs_full'
echo "[A2 fc full] recovered teacher uuid OK"

if [[ ! -f data/processed/lapai/n320_latlons.npy ]]; then
  echo "[A2 fc full] extract N320 lat/lons"
...
mkdir -p data/processed/trackA_prune_full/forecasts
INITS=(20230101 20230108 20230115 20230122 20230129)

for init in "${INITS[@]}"; do
  out="data/processed/trackA_prune_full/forecasts/${init}_00Z.nc"
  if [[ -f "${out}" ]]; then
    echo "[A2 fc full] skip existing ${out}"
    continue
  fi
  echo "[A2 fc full] forecasting ${init}"
  python -u scripts/run_phase0_forecast.py \
    --config configs/trackA_prune_full.yaml \
    --init "${init}" \
    --lead-time 240 \
...
echo "[A2 fc full] scoring -> TRACKA_PRUNE_FULL_SCORECARD.json"
python -u - <<'PY'
...
from evaluation.gcs_era5_truth import (
    GCS_OPTS, era5_gcs_path, normalize_lon,
